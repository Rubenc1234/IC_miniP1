...
BZIP2 & 654 MB & \textbf{1.44:1} & 66 s & 40 s & 7.8 MB \\
...
\begin{enumerate}
    \item \textbf{Máxima compressão}: BZIP2 com rácio 1.44:1, tempo aceitável (66 s) e consumo moderado de RAM;
    \item \textbf{Máxima velocidade}: ZSTD com rácio 1.28:1, tempo excelente (2 s) e consumo baixo de RAM.
\end{enumerate}
...
Implementação clássica de codificação de Huffman, uma técnica de codificação entrópica que oferece compressão eficiente com tabela de códigos pré-calculada. As principais características da implementação são:

\begin{itemize}
    \item \textbf{Look-Up Table (LUT)}: Tabela de códigos pré-calculada para codificação eficiente em tempo $O(1)$;
    \item \textbf{Tabela de frequências}: Cada bloco inclui a tabela de frequências (256 × 4 bytes = 1 KB), permitindo a reconstrução da árvore de Huffman;
    \item \textbf{Buffer de 64 bits}: Acumulação eficiente de bits, emitindo bytes completos para o fluxo de saída à medida que ficam disponíveis;
    \item \textbf{Descodificação independente}: Cada bloco pode ser descodificado independentemente dos restantes.
\end{itemize}
...
\begin{itemize}
    ...
    \textit{Maximiza a taxa de compressão}
\end{itemize}
\end{tcolorbox}
...
\begin{table}[H]
    \centering
    \caption{Impacto do Tamanho de Bloco no Desempenho do Codec}
...
\begin{itemize}
    \item \textbf{Taxa de compressão}: Blocos maiores permitem estatísticas mais representativas para a codificação entrópica, melhorando o rácio de compressão. A diferença entre 64 KB e 1 MB é de aproximadamente 28 MB (4.2\%) no modo BEST;
    \item \textbf{Tempo de processamento}: Blocos maiores reduzem o \textit{overhead} por bloco (tabelas de frequências, metadados), resultando em tempos globalmente inferiores;
    \item \textbf{Consumo de RAM}: O aumento de 3.9 MB para 6.6 MB é negligenciável para sistemas modernos;
    \item \textbf{Retornos decrescentes}: A curva de melhoria estabiliza a partir de 512 KB, sugerindo que blocos superiores a 1 MB teriam ganhos marginais.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Superação dos Benchmarks}: O codec desenvolvido atingiu um rácio de compressão de 1:50:1, superando todos os compressores de uso geral testados, incluindo o BZIP2 (1.44:1).
