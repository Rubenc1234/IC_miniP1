...
[Bloco 1: MSB comprimido | LSB (raw/RLE)]
[Bloco 2: MSB comprimido | LSB (raw/RLE)]
...
[Bloco N: MSB comprimido | LSB (raw/RLE)]
\end{verbatim}
...
Implementação clássica com tabela de frequências calculada por bloco. O cabeçalho de cada bloco inclui a tabela de frequências (256 × 4 bytes = 1 KB), permitindo a descodificação independente de blocos.

\paragraph{Codificação Aritmética (Estática)}
Implementação de precisão inteira com modelo estatístico de ordem-0. Utiliza aritmética de 32 bits para evitar problemas de \textit{underflow}.

A Tabela~\ref{tab:entropy_bench} compara o desempenho dos dois algoritmos:
...
A Codificação Aritmética permitiu uma redução adicional de $\approx 2$ MB, o que se explica teoricamente pela sua capacidade de alocar um número fracionário de bits por símbolo. Contudo, este ganho marginal (0.3\%) implicou um aumento de 36\% no tempo de execução.
...
\subsubsection{Implementação de RLE com Fallback}

Implementou-se uma estratégia oportunística:

\begin{enumerate}
    \item O algoritmo procura sequências de zeros consecutivos (indicadores de \textit{sparsity} em alguns modelos);
    \item Se o tamanho comprimido for \textbf{superior} ao original, o bloco é armazenado em modo \textit{Raw} (cópia direta);
    \item Um bit de flag no início de cada bloco indica o modo utilizado.
\end{enumerate}

\textbf{Resultado Prático}: Nos testes realizados com o modelo Qwen2-0.5B, o mecanismo de \textit{fallback} foi ativado em 100\% dos blocos, resultando em armazenamento direto para todo o canal LSB. Este resultado era esperado, dado que modelos densos (não esparsos) não apresentam sequências significativas de zeros.
...
\begin{itemize}
    \item \textbf{Modo FAST}: Huffman (MSB) + Raw (LSB) \\
    \textit{Prioriza velocidade de processamento}
    
    \item \textbf{Modo BEST}: Aritmético (MSB) + RLE/Raw (LSB) \\
    \textit{Prioriza minimização do tamanho final}
\end{itemize}
\end{tcolorbox}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccc}
        ...
        & Fast & 943 MB & 634 MB & 1.488:1 & 10.5 s & 9.4 s \\
        & Best & 943 MB & 632 MB & 1.493:1 & 15.9 s & 21.2 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_1.safetensors}} 
        & Fast & 421 MB & 374 MB & 1.124:1 & 8.2 s & 5.9 s \\
        & Best & 421 MB & 374 MB & 1.126:1 & 10.0 s & 15.0 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_2.safetensors}} 
        & Fast & 437 MB & 390 MB & 1.121:1 & 8.7 s & 6.0 s \\
        & Best & 437 MB & 389 MB & 1.123:1 & 10.7 s & 15.7 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_3.safetensors}} 
        & Fast & 682 MB & 608 MB & 1.122:1 & 13.4 s & 9.7 s \\
        & Best & 682 MB & 607 MB & 1.124:1 & 16.3 s & 24.7 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_4.safetensors}} 
        & Fast & 4.6 GB & 3.1 GB & 1.495:1 & 54.9 s & 47.0 s \\
        & Best & 4.6 GB & 3.1 GB & 1.501:1 & 81.0 s & 108.0 s \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        \textbf{Codec (Fast)} & \textbf{1.49:1} & 10.5 s & 9.4 s \\
        \textbf{Codec (Best)} & \textbf{1.49:1} & 15.9 s & 21.2 s \\
        \bottomrule
    \end{tabular}
\end{table}
...
O modo \textbf{Fast} oferece um compromisso excelente:
\begin{itemize}
    \item \textbf{3.5× mais lento que ZSTD}, mas com rácio 15\% superior;
    \item \textbf{9× mais rápido que BZIP2}, com rácio 3.5\% superior.
\end{itemize}

O modo \textbf{Best} é adequado para cenários de arquivo onde o tempo não é crítico.

\subsubsection{Escalabilidade}

Os tempos de processamento escalam linearmente com o tamanho do ficheiro:
\begin{itemize}
    \item \texttt{model.safetensors} (943 MB): 10.5 s
    \item \texttt{model\_4.safetensors} (4.6 GB): 54.9 s
\end{itemize}

O rácio tempo/tamanho mantém-se aproximadamente constante ($\approx$ 11 MB/s no modo Fast), demonstrando boa escalabilidade.
...
\begin{enumerate}
    ...
    \item \textbf{Eficiência Computacional}: O modo Fast processa aproximadamente 90 MB/s, oferecendo um compromisso favorável entre compressão e velocidade.
...
\begin{itemize}
    ...
    \item \textbf{Canal LSB}: A compressão oportunística (RLE) não foi eficaz para modelos densos. Técnicas mais sofisticadas, como a exploração de correlações intra-tensor, poderiam melhorar os resultados.
