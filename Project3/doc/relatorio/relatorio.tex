...
\begin{itemize}
    \item \textbf{CPU}: [Especificar processador]
    \item \textbf{RAM}: [Especificar memória]
    \item \textbf{Armazenamento}: [Especificar tipo de disco]
    \item \textbf{Sistema Operativo}: [Especificar SO]
\end{itemize}
...
[Bloco 1: MSB comprimido | LSB raw]
[Bloco 2: MSB comprimido | LSB raw]
...
[Bloco N: MSB comprimido | LSB raw]
\end{verbatim}

O cabeçalho JSON original é preservado integralmente para garantir a compatibilidade com ferramentas existentes.
...
\paragraph{Codificação rANS (\textit{Asymmetric Numeral Systems})}
Implementação de rANS (\textit{range Asymmetric Numeral Systems}), uma técnica moderna de codificação entrópica que oferece compressão próxima da entropia com desempenho computacional superior ao da codificação aritmética tradicional. As principais características da implementação são:

\begin{itemize}
    \item \textbf{Precisão}: Utiliza uma tabela de frequências normalizada para $2^{12}$ (4096) símbolos totais;
    \item \textbf{Estado}: Mantém um estado de 64 bits com limites $[2^{32}, 2^{40})$ para garantir precisão;
    \item \textbf{Codificação inversa}: Os símbolos são codificados em ordem reversa, permitindo descodificação em ordem direta.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Huffman (LUT) & 633.08 MB & \textbf{4.0 s} & \textbf{3.9 s} \\
        rANS & \textbf{631.13 MB} & 4.8 s & 4.5 s \\
        \bottomrule
    \end{tabular}
\end{table}

A codificação rANS permitiu uma redução de $\approx 2$ MB comparativamente ao Huffman, o que se explica teoricamente pela sua capacidade de alocar um número fracionário de bits por símbolo. O decoder Huffman foi otimizado com uma \textit{Look-Up Table} (LUT) de 12 bits, permitindo descodificar múltiplos bits por operação e atingindo tempos competitivos com o rANS.

\subsection{Estratégia para o Canal LSB}

O canal LSB, com entropia de 7.96 bits/byte, apresenta características de ruído quase uniforme. Neste contexto, algoritmos de compressão entrópica tradicionais não só são ineficazes como podem resultar em \textbf{expansão} dos dados.

\subsubsection{Armazenamento Direto (Raw)}

Após análise experimental, optou-se por uma estratégia de armazenamento direto para o canal LSB:
...
\textbf{Decisão de Engenharia}: Os dados do canal LSB são copiados diretamente para o ficheiro comprimido sem qualquer transformação. Esta abordagem simplifica significativamente a implementação e maximiza a velocidade de processamento sem comprometer a taxa de compressão global.
...
\begin{itemize}
    ...
    \textit{Desempenho equilibrado entre codificação e descodificação}
    
    \item \textbf{Modo BEST}: rANS (MSB) + Raw (LSB) \\
    \textit{Maximiza a taxa de compressão ($\approx$ 0.3\% melhor)}
\end{itemize}
\end{tcolorbox}
...
\begin{table}[H]
    ...
    \begin{tabular}{lrr}
        ...
        \texttt{model\_1.safetensors} & 421 MB & Modelo auxiliar 1 \\
        \texttt{model\_2.safetensors} & 437 MB & Modelo auxiliar 2 \\
        \texttt{model\_3.safetensors} & 682 MB & Modelo auxiliar 3 \\
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccccc}
        ...
        & Fast & 943 MB & 633 MB & 1.488:1 & 4.0 s & 3.9 s \\
        & Best & 943 MB & 631 MB & 1.493:1 & 4.8 s & 4.5 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_1.safetensors}} 
        & Fast & 421 MB & 374 MB & 1.124:1 & 1.6 s & 2.0 s \\
        & Best & 421 MB & 373 MB & 1.125:1 & 2.1 s & 2.1 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_2.safetensors}} 
        & Fast & 437 MB & 389 MB & 1.121:1 & 1.6 s & 2.1 s \\
        & Best & 437 MB & 389 MB & 1.123:1 & 2.3 s & 2.1 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_3.safetensors}} 
        & Fast & 682 MB & 607 MB & 1.122:1 & 2.9 s & 3.2 s \\
        & Best & 682 MB & 606 MB & 1.123:1 & 3.6 s & 3.2 s \\
        \midrule
        \multirow{2}{*}{\texttt{model\_4.safetensors}} 
        & Fast & 4.6 GB & 3.1 GB & 1.495:1 & 20.4 s & 25.1 s \\
        & Best & 4.6 GB & 3.1 GB & 1.501:1 & 26.0 s & 25.1 s \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        \textbf{Codec (Fast)} & 1.49:1 & 4.0 s & 3.9 s \\
        \textbf{Codec (Best)} & 1.49:1 & 4.8 s & 4.5 s \\
        \bottomrule
    \end{tabular}
\end{table}
...
O codec desenvolvido \textbf{superou todos os compressores de uso geral testados}, atingindo um rácio de 1.49:1 no modelo principal, comparado com 1.44:1 do melhor compressor genérico (BZIP2). Esta melhoria de 3.5\% representa uma poupança adicional de aproximadamente 33 MB no ficheiro de 943 MB.
...
\begin{itemize}
    \item \textbf{model.safetensors} e \textbf{model\_4.safetensors}: Rácio $\approx$ 1.49:1
    \item \textbf{model\_1/2/3.safetensors}: Rácio $\approx$ 1.12:1
\end{itemize}
...
Ambos os modos oferecem desempenho excelente após a otimização do decoder Huffman com LUT:
\begin{itemize}
    \item \textbf{Modo Fast}: Codificação em 4.0 s e descodificação em 3.9 s (\textbf{ciclo completo < 8 s});
    \item \textbf{Modo Best}: Codificação em 4.8 s e descodificação em 4.5 s, com melhor compressão.
\end{itemize}

Comparativamente aos compressores de uso geral:
\begin{itemize}
    \item \textbf{Velocidade similar ao ZSTD} na codificação, mas com rácio 15\% superior (1.49:1 vs 1.29:1);
    \item \textbf{23× mais rápido que BZIP2} na codificação e \textbf{12× mais rápido} na descodificação.
\end{itemize}
...
\begin{itemize}
    \item \texttt{model.safetensors} (943 MB): 4.0 s cod. + 3.9 s dec. (Fast)
    \item \texttt{model\_4.safetensors} (4.6 GB): 20.4 s cod. + 25.1 s dec. (Fast)
\end{itemize}

O \textit{throughput} mantém-se aproximadamente constante ($\approx$ 185--235 MB/s), demonstrando excelente escalabilidade.
...
\begin{enumerate}
    ...
    \item \textbf{Superação dos Benchmarks}: O codec desenvolvido atingiu um rácio de compressão de 1.49:1, superando todos os compressores de uso geral testados, incluindo o BZIP2 (1.44:1).
    
    \item \textbf{Eficiência Computacional}: O decoder Huffman otimizado com LUT de 12 bits atinge \textit{throughput} de $\approx$ 240 MB/s, permitindo ciclos completos de codificação + descodificação em menos de 8 segundos para ficheiros de 1 GB.
