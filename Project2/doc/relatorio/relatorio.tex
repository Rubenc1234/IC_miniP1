...
Para a análise, foi utilizado um ficheiro de áudio estéreo de 12 segundos (2.1 MB). Para efeitos de comparação o mesmo ficheiro foi comprimido utilizando o codec \textbf{FLAC} (\textit{Free Lossless Audio Codec}) com as definições padrão.

\subsubsection{Resultados Comparativos}

\begin{table}[htbp]
\centering
\caption{Comparação de Compressão: O Nosso Codec vs. FLAC}
\label{tab:compressao_audio_comparativa}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Codec} & \textbf{Tamanho Comprimido} & \textbf{Taxa de Compressão} & \textbf{Poupança de Espaço} \\ \midrule
Original (WAV) & 2.10 MB & 1.00 : 1 & 0\% \\
\textbf{O Nosso Codec} & \textbf{1.60 MB} & \textbf{1.31 : 1} & \textbf{23.8\%} \\
FLAC (Standard) & 1.15 MB & 1.83 : 1 & 45.2\% \\
\bottomrule
\end{tabular}
\end{table}
...
Como se observa na Tabela~\ref{tab:compressao_audio_comparativa}, o nosso codec atinge uma redução de tamanho de cerca de 24\%, enquanto o FLAC atinge cerca de 45\%.

Esta diferença de desempenho é esperada e justifica-se pela complexidade do modelo de predição:
\begin{itemize}
    \item \textbf{O Nosso Codec:} Utiliza um preditor linear **fixo** de 1ª ordem ($P[n] = x[n-1]$). Este modelo não se adapta às variações espectrais complexas do áudio, gerando resíduos com uma variância ainda considerável.
    \item \textbf{FLAC:} Utiliza **Predição Linear Adaptativa (LPC)**. Para cada bloco, o FLAC calcula os coeficientes de um polinómio que melhor se ajusta àquela onda sonora específica. Isto resulta em resíduos muito mais pequenos e descorrelacionados, que o codificador Rice (semelhante ao Golomb) comprime com muito mais eficácia.
\end{itemize}
Conclui-se que, para áudio, a adaptação do parâmetro \texttt{m} (que implementámos com sucesso) é importante, mas a adaptação do \textbf{modelo de predição} (que o FLAC faz e nós não) é o fator determinante para uma alta taxa de compressão.
...
A eficácia do codec foi testada com a imagem \texttt{airplane.ppm} (786.4 kB) e comparada com \textit{standards} de compressão \textit{lossless}: o **PNG** (com compressão máxima) e o **FLIF** (\textit{Free Lossless Image Format}), um dos codecs mais recentes e eficientes.

\begin{table}[htbp]
\centering
\caption{Comparação de Compressão: O Nosso Codec vs. Standards}
\label{tab:compressao_imagem}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Codec} & \textbf{Tamanho Comprimido} & \textbf{Taxa de Compressão} & \textbf{Poupança} \\ \midrule
Original (PPM) & 786.4 kB & 1.00 : 1 & 0\% \\
PNG (Max Compression) & 495.2 kB & 1.59 : 1 & 37.0\% \\
\textbf{O Nosso Codec} & \textbf{132.7 kB} & \textbf{5.92 : 1} & \textbf{83.1\%} \\
FLIF (Lossless) & 129.5 kB & 6.07 : 1 & 83.5\% \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Discussão}
Os resultados na Tabela~\ref{tab:compressao_imagem} são notáveis. O nosso codec supera largamente o formato PNG e aproxima-se do desempenho do FLIF.

Ao contrário do áudio, onde ficámos atrás do standard, aqui superámos o codec comum (PNG). Isto deve-se à escolha do **preditor MED** (usado no standard JPEG-LS).
\begin{itemize}
    \item O PNG usa um preditor simples (Paeth) e codificação Huffman/Deflate.
    \item O nosso codec usa o preditor MED (que deteta arestas) e Golomb adaptativo. Para imagens fotográficas ou com gradientes suaves (como a \texttt{airplane.ppm}), o modelo MED gera resíduos extremamente baixos (muitas vezes 0 ou $\pm 1$), um cenário onde o código Golomb é teoricamente ótimo e superior aos métodos genéricos do PNG.
\end{itemize}
