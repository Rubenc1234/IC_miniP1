...
Esta parte focou-se na implementação de uma classe C++ para a codificação Golomb, conforme solicitado no enunciado. Esta é uma técnica de codificação entrópica eficiente para fontes com distribuições geométricas. A classe foi desenhada para ser a base dos codecs \textit{lossless} das partes seguintes.
...
\begin{enumerate}
    \item Um inteiro não-negativo $i$ é dividido em duas partes: um quociente $q$ e um resto $r$.
    \item As fórmulas para $q$ e $r$ são:
        $$ q = \left\lfloor \frac{i}{m} \right\rfloor \quad \text{e} \quad r = i - qm $$ 
    \item O quociente $q$ é codificado em código unário (uma sequência de $q$ bits '1' seguida de um bit '0').
...
\begin{enumerate}
    \item \textbf{Leitura e Cabeçalho:} O ficheiro WAV é lido para a memória. O codificador escreve um cabeçalho no ficheiro de saída contendo os metadados (\texttt{samplerate}, \texttt{numChannels}, \texttt{numFrames}), mas \textbf{não} escreve um \texttt{m} global.
    
    \item \textbf{Processamento por Blocos:} O áudio é processado em blocos (foi usado um \texttt{blockSize} de 4096 tramas). Para cada bloco:
    \begin{itemize}
        \item \textbf{Passagem 1 (do Bloco):} É feita uma passagem interna no bloco para calcular todos os resíduos.
            ...
            \end{itemize}
        \item \textbf{Cálculo de \texttt{m} Ótimo:} A função \texttt{calculate\_optimal\_m} é chamada para os vetores de resíduos (um para mono, dois para estéreo). (Ver Secção 4.1.2 para detalhes).
...
A teoria (como a da Figura \ref{fig:teoria_m}) define \texttt{m} em função de parâmetros estatísticos teóricos ($\alpha$ ou $p=P(x=0)$). Na prática, não conhecemos estes parâmetros, mas podemos medi-los através da média dos nossos dados (os resíduos).

Os nossos resíduos com sinal (e.g., -50, 100, 2) seguem uma distribuição de Laplace. A nossa classe \texttt{Golomb} usa o mapeamento \texttt{INTERLEAVING} para os converter em inteiros não-negativos $i$. A teoria de compressão (usada em \textit{standards} como o JPEG-LS) demonstra que o \texttt{m} ótimo pode ser estimado diretamente da média ($\mu$) destes valores $i$ mapeados, usando a aproximação:
$$ m \approx \mu \times \ln(2) $$

\paragraph{Implementação}
A nossa função \texttt{calculate\_optimal\_m} implementa esta lógica:
\begin{enumerate}
    \item Calcula a soma dos resíduos após o mapeamento \texttt{INTERLEAVING} (`sum += (res >= 0) ? (2.0 * res) : (2.0 * abs(res) - 1.0)`).
    \item Calcula a média (`mean = sum / residuals.size()`).
    \item Estima \texttt{m} usando a fórmula: `m = round(mean * log(2))` (onde `log(2)` da \texttt{<cmath>} é $\ln(2)$).
\end{enumerate}
...
./bin/audio_encoder img/sample.wav out/sample_compressed.bin
\end{lstlisting}

\subsection{Descodificador \texttt{audio\_decoder}}
O descodificador (\texttt{audio\_decoder.cpp}) inverte o processo de forma síncrona, lendo o \texttt{m} de cada bloco antes de o descodificar.
...
./bin/audio_decoder out/sample_compressed.bin out/sample_restaurado.wav
\end{lstlisting}

\subsection{Análise de Desempenho e Compressão}
A compressão foi avaliada num ficheiro de teste estéreo (2.1 MB).
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Ficheiro} & \textbf{Original (WAV)} & \textbf{Comprimido (bin)} & \textbf{Taxa de Compressão} \\ \midrule
sample.wav & 2.1 MB & 1.7 MB &  1.24 : 1 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Análise dos Resultados}
A compressão alcançada, mesmo com um \texttt{m} adaptativo, é muito modesta (1.24:1). Este resultado não indica um erro no codec Golomb, mas sim a ineficácia do preditor de 1ª ordem ($P[n] = x[n-1]$). Um sinal de áudio é complexo, e assumir que a próxima amostra é igual à anterior é uma previsão fraca que gera resíduos médios muito grandes. O \texttt{m} ótimo calculado é, por isso, muito elevado, e a codificação Golomb não é eficiente para comprimir números grandes. O ganho de compressão é marginal, pois o preditor falha em descorrelacionar eficazmente o sinal.


\section{Parte IV: Codec Imagem Lossless (Grayscale)}

Aplicando a mesma arquitetura de \texttt{m} adaptativo por bloco, foi desenvolvido um codec \textit{lossless} para imagens em escala de cinza, conforme a Parte IV do enunciado.

\subsection{Codificador \texttt{lossless\_image\_encoder}}

\subsubsection{Algoritmo de Codificação}
O codificador (\texttt{lossless\_image\_encoder.cpp}) implementa uma lógica adaptativa por blocos de 16x16.
...
\begin{enumerate}
    ...
    \item \textbf{Escrita do Cabeçalho:} Um "magic number" (\texttt{"GOL1"}) é escrito, seguido das dimensões da imagem. Nenhum \texttt{m} global é guardado.
    \item \textbf{Processamento por Blocos (16x16):} Para cada bloco:
    \begin{itemize}
        \item \textbf{Passagem 1 (Análise):} O codificador calcula os resíduos de predição para o bloco. É usado o preditor MED (Median Edge-Detection), que estima o píxel atual $P(x,y)$ com base nos vizinhos Oeste ($a$), Norte ($b$) e Noroeste ($c$). Os resíduos ($e = \text{píxel} - \text{predição}$) são guardados num vetor temporário.
        \item \textbf{Cálculo de \texttt{m}:} O \texttt{m} ótimo para os resíduos do bloco é calculado com \texttt{calculate\_optimal\_m}, tal como explicado na Secção 4.1.2.
...
\subsubsection{Sintaxe e Exemplos}
O codificador calcula \texttt{m} automaticamente, não sendo um argumento.
\begin{lstlisting}[language=bash, caption=Sintaxe de Uso do lossless\_image\_encoder]
./bin/lossless_image_encoder <imagem_entrada.ppm> <ficheiro_saida.gol>
\end{lstlisting}
\begin{lstlisting}[language=bash, caption=Exemplo de Codificação de Imagem]
./bin/lossless_image_encoder img/airplane.ppm out/airplane_compressed.gol
\end{lstlisting}

\subsection{Descodificador \texttt{lossless\_image\_decoder}}
O descodificador (\texttt{lossless\_image\_decoder.cpp}) inverte o processo, lendo o \texttt{m} de cada bloco.

\begin{lstlisting}[language=bash, caption=Sintaxe de Uso do lossless\_image\_decoder]
./bin/lossless_image_decoder <ficheiro_entrada.gol> <imagem_saida.ppm>
\end{lstlisting}
\begin{lstlisting}[language=bash, caption=Exemplo de Descodificação de Imagem]
./bin/lossless_image_decoder out/airplane_compressed.gol out/airplane_restaurada.ppm
\end{lstlisting}


\subsection{Análise de Desempenho e Compressão}

A eficácia do codec foi testada com a imagem \texttt{airplane.ppm} (786.4 kB) e comparada com \textit{standards} de compressão \textit{lossless}.

\begin{table}[htbp]
\centering
\caption{Análise de Compressão do Codec de Imagem (vs. Standard)}
\label{tab:compressao_imagem}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Codec} & \textbf{Original} & \textbf{Comprimido} & \textbf{Taxa de Compressão} \\ \midrule
\textbf{Codec Golomb-MED (O nosso)} & 786.4 kB & \textbf{132.7 kB} & \textbf{5.93 : 1} \\
PNG (Compressão Máxima) & 786.4 kB & 495.2 kB & 1.59 : 1 \\
FLIF (Lossless) & 786.4 kB & 129.5 kB & 6.07 : 1 \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Análise dos Resultados}
Os resultados são notavelmente positivos. O nosso codec \textit{lossless} (Taxa 5.93:1) supera largamente o \textit{standard} PNG (1.59:1) e atinge um desempenho muito próximo de codecs modernos como o FLIF.

A razão para esta compressão excecional, especialmente quando comparada com a compressão modesta do nosso codec de áudio, reside inteiramente na eficácia do preditor MED (Median Edge-Detection). Um sinal de imagem 2D tem uma correlação espacial muito elevada, e o preditor MED é extremamente eficaz a explorar essa correlação. As previsões são tão precisas que os resíduos gerados são maioritariamente nulos ou muito pequenos, permitindo que o \texttt{calculate\_optimal\_m} escolha um \texttt{m} muito baixo (e.g., \texttt{m=1}), para o qual a codificação Golomb é extremamente eficiente.
